...
| **API Transporte — GCBA** (apitransporte.buenosaires.gob.ar) | Colectivos layer (live AMBA bus GPS, `/colectivos/vehiclePositionsSimple`) and Buenos Aires Ecobici GBFS (`/ecobici/gbfs/*`). BYOK: free `client_id`/`client_secret`, server-side only | [GCBA open-data terms](https://data.buenosaires.gob.ar/acerca/) — attribution; respect the per-key rate limits | "API Transporte — Gobierno de la Ciudad de Buenos Aires" |
| **SMN — Servicio Meteorológico Nacional (Argentina)** (ssl.smn.gob.ar open data) | Clima SMN layer: current observations of ~120 stations (temperature, wind, humidity, pressure) joined to the SMN station list; keyless | [SMN open data](https://www.smn.gob.ar/descarga-de-datos) — public data, cite SMN | "Servicio Meteorológico Nacional — datos abiertos" |
| **CAMMESA** (api.cammesa.com) | Red Eléctrica layer: live demand per region + national generation mix, 5-minute samples; keyless | Public real-time data published by CAMMESA; cite CAMMESA | "CAMMESA" |
| **CelesTrak NAME queries (synthetic `argentina` group)** | ARGENTINA satellite class: SAOCOM, ARSAT, ÑuSat, BugSat, CubeBug assembled from NAME searches | Same as CelesTrak above | "CelesTrak (celestrak.org), Dr. T.S. Kelso" |
| **Buenos Aires Ciudad — Cámaras fijas de control vehicular** (bundled `local_data/caba_radares/`) | Fotomultas CABA layer: 224 fixed enforcement cameras (snapshot 2025-07-23) | CC-BY-2.5-AR | "Buenos Aires Ciudad — data.buenosaires.gob.ar" |
| **RainViewer** (api.rainviewer.com + tilecache.rainviewer.com) | Radar de lluvia layer: animated global precipitation radar composite, 10-minute frames, keyless | [RainViewer API terms](https://www.rainviewer.com/api.html) — free public tier, attribution requested | "RainViewer" |
| **CONAE — Focos de calor** (focosdecalor.conae.gov.ar GeoServer WFS) | Focos de calor layer: GOES-19 hotspots, last 24 h, 10-minute cadence; keyless | Public CONAE geoservice; cite CONAE | "CONAE" |
| **Edesur — outage report** (ed.edesur.com.ar, undocumented JSON behind the public widget) | Cortes de luz layer: programmed/active outages with coordinates and customers affected | Public page data, no terms published; endpoint may change | "Edesur" |
| **Secretaría de Energía — Precios en surtidor** (datos.energia.gob.ar CSV, Res. 314/2016) | Precios de nafta layer: last reported price per station × product | CC-BY | "Secretaría de Energía" |
| **Ministerio de Seguridad PBA (ArcGIS REST) + Datos Provincia Abierta (GitHub)** (bundled `local_data/pba_comisarias/`, merged into `caba_radares/`) | Comisarías PBA layer (2 173 points) and the provincial half of Fotomultas BA (252 radars) | Public map service / public repository | "Ministerio de Seguridad PBA", "Datos Provincia Abierta" |
| **IDE Transporte — Estaciones ferroviarias con servicio 2022** (bundled `local_data/tren_estaciones/`) | Estaciones de tren layer (464 points) | CC-BY | "IDE Transporte — Ministerio de Transporte" |
| **Telpin (Pinamar) / Catedral Alta Patagonia–Varitech** (config/cctv_sources.argentina.json) | CCTV layer: 7 HLS beach cams in Pinamar, 4 JPG cams on Cerro Catedral | Public streams on the operators' sites; no explicit licence — courtesy use | "Telpin", "Catedral Alta Patagonia" |
| **INA — Sistema de Información y Alerta Hidrológico (a5 API)** (alerta.ina.gob.ar) | Ríos INA layer: hydrometric gauges with latest height and alert/evacuation thresholds (Prefectura, SHN, INA, provincial networks) | Public API, no key; cite INA and the data owners | "Instituto Nacional del Agua" |
| **SMN — alertas CAP** (ssl.smn.gob.ar/CAP/AR.php + feeds/CAP/xml_generados) | Alertas meteorológicas layer: official CAP 1.2 alert polygons (tormentas, viento, zonda, nevadas, lluvias…) | Public SMN feed; cite SMN | "Servicio Meteorológico Nacional" |
| **Vialidad Nacional (SIG-Vial) + rutas.ar** (bundled `local_data/vialidad_rutas/`, live `rutas.ar/api/estado`) | Estado de rutas layer: national network geometry (simplified) colored by live route status | Vialidad: public national data. rutas.ar: unofficial public scrape (30 req/min) | "Vialidad Nacional" / "rutas.ar" |
| **GPS Bahía** (gpsbahia.com.ar, unofficial web-app endpoint with session cookie) | Colectivos layer, Bahía Blanca half: live bus positions for 24 lines | Municipal public app, no published terms; may break without notice | "GPS Bahía — Municipio de Bahía Blanca" |
| **Buenos Aires Ciudad — epok (Mapa Interactivo BA backend)** (live: subte; bundled: salud, bomberos, servicios) | Subte layer (stations + lines with live `Estado`), Salud pública BA, Bomberos BA, Puntos de interés CABA (17 317 points from ~75 city layers: cultura, deporte, educación, salud, social, seguridad, movilidad, turismo, memoria, servicios — filtered by DISPLAY chips) | Public city map service, CC-BY GCBA | "Mapa Interactivo BA — Buenos Aires Ciudad" |
| **AGP ePuertos** (api.agp-ports.gob.ar) | Puerto de Buenos Aires layer: ship calls at berth with status/ETA/ETD | Public port authority API, no terms published | "AGP ePuertos" |
| **APrA — calidad de aire** (cdn.buenosaires.gob.ar CSV) | Calidad de aire CABA layer: latest hourly CO/NO2/PM10 per station | CC-BY-2.5-AR | "APrA — Buenos Aires Ciudad" |
| **Datos Abiertos PBA** (catalogo.datos.gba.gob.ar CSVs, bundled) | Provincial half of Salud pública BA, Bomberos BA and Estaciones de tren | CC-BY 4.0 | "Datos Abiertos Provincia de Buenos Aires" |
| **RENABAP 2023** (datos.gob.ar → archivo.infraestructura.gob.ar `20231205_info_publica.geojson`, bundled `local_data/renabap_amba/`) | Barrios populares layer: every RENABAP polygon in CABA + Provincia de Buenos Aires, with families, dwellings, area, kind, tenure and service access shown on hover | Public national registry (open data) | "RENABAP 2023 — Secretaría de Integración Socio Urbana" |
| **Buenos Aires Ciudad — ciclovías** (cdn.buenosaires.gob.ar, bundled) | Ciclovías CABA layer | CC-BY-2.5-AR | "Buenos Aires Ciudad" |
| **IDEBA nodo La Plata** (geoserver-nodo2.ideba.gba.gob.ar WFS, bundled high-risk subset) | Riesgo de inundación La Plata layer (6 047 street segments) | Public municipal data | "Municipalidad de La Plata" |
| **USIG mapcache tiles** (tiles1.usig.buenosaires.gob.ar TMS, brokered by `/api/usig/tiles` with retries because the city renders tiles on demand) | Mapa de ruido CABA, Arroyos y cuencas CABA, Fotos aéreas históricas CABA (1940/1965/1978/2004/2006/2008/2009/2014/2017) and Mapas temáticos CABA (14 rasters) imagery layers | Public city geoservice | "USIG — Buenos Aires Ciudad" |
| **Datos Abiertos PBA — Establecimientos educativos** (bundled `local_data/pba_escuelas/`) | Escuelas PBA layer (19 848 points with nivel, sector, matrícula) | CC-BY 4.0 | "Datos Abiertos Provincia de Buenos Aires" |
| **IDEBA GeoServer WFS** (geoserver.ideba.gba.gob.ar, bundled `local_data/pba_terminales/`) | Terminales y peajes PBA layer (105 bus terminals + 91 toll plazas) | Public provincial geoservice | "IDEBA" |
| **IDEs provinciales (GeoServer WFS)** — IDECOR, IDE Corrientes, IDE Jujuy, IDE Río Negro, IDE Salta, SITU Santa Cruz, IDE Tierra del Fuego, IDE Chaco, IDE La Pampa, GeoSPLAN Tucumán, GeoCloud Salta (bundled `local_data/prov_salud/`, `prov_seguridad/`) | Salud pública provincias and Comisarías y bomberos provincias layers | Public provincial geoservices | "IDEs provinciales" |
| **Hidrocarburos Neuquén / IDE La Pampa / IDE TDF (WFS)** (bundled `local_data/pozos_hidrocarburos/`) | Pozos de petróleo y gas layer (Neuquén 20 496 + La Pampa + Tierra del Fuego) | Public provincial geoservices | "Subsecretaría de Energía de Neuquén" et al. |
| **Mapa Educativo Nacional (mapa.educacion.gob.ar WFS)** (bundled `local_data/escuelas_argentina/`) | Escuelas resto del país layer (every school outside Buenos Aires) | Public national geoservice — DIEE | "Mapa Educativo Nacional" |
| **Datos Abiertos Mar del Plata** (datos.mardelplata.gob.ar, bundled `local_data/mdp_paradas/` + `mdp_recorridos/`) | Paradas (10 081) and Recorridos (128 lines) colectivo Mar del Plata layers | Public municipal open data | "Municipalidad de General Pueyrredón" |
| **Windy Webcams API v3** | Optional CCTV source pack (default: every public Windy webcam in Argentina). BYOK, server-side only; preview URLs re-resolved because free-tier tokens expire in 10 min | [Windy Webcams API terms](https://api.windy.com/webcams/terms) — attribution to windy.com and the broadcaster | "Windy Webcams" + link to each broadcaster |
| **Ciudad de Mendoza — cámaras públicas** (camarasmunicapital.ciudaddemendoza.gov.ar) | Bundled CCTV pack `config/cctv_sources.mendoza.json` (Plaza Independencia, Terraza Municipal) | Public municipal streams (one published as CC BY 4.0) | "Ciudad de Mendoza" |
